...
impl<R: Runtime> NetworkMonitor<R> {
    ...
    async fn check_network_changes(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if gateway_changed || should_periodic_check {
            let check_reason = if gateway_changed {
                "gateway changed"
            } else {
                "periodic check"
            };
            println!(
                "[WiFi Debug] Checking network status (reason: {})",
                check_reason
            );
            ...
            if network_status.is_tollgate {
                ...
                // Clear session if no tollgate detected
                {
                    ...
                    if session.is_some() {
                        println!(
                            "[Tollgate Payment] No tollgate detected, clearing active session"
                        );
                        *session = None;
                    }
                }
    ...
    async fn get_full_network_status(
        &self,
        ...
        println!(
            "[WiFi Debug] get_full_network_status: Gateway IP: {:?}",
            gateway_ip
        );
        ...
        println!(
            "[WiFi Debug] get_full_network_status: MAC address: {:?}",
            mac_address
        );
        ...
        println!(
            "[WiFi Debug] get_full_network_status: WiFi info: {:?}",
            current_wifi
        );
        ...
        let (is_tollgate, tollgate_advertisement) = if let Some(ref gateway) = gateway_ip {
            println!(
                "[WiFi Debug] get_full_network_status: Checking tollgate at gateway: {}",
                gateway
            );
            ...
            match detector.check_tollgate_at_gateway(gateway).await {
                ...
                Err(e) => {
                    println!(
                        "[WiFi Debug] get_full_network_status: Tollgate detection failed: {:?}",
                        e
                    );
                    (false, None)
                }
            ...
            println!(
                "[WiFi Debug] get_full_network_status: No gateway, skipping tollgate detection"
            );
            (false, None)
        };
        ...
        println!(
            "[WiFi Debug] get_full_network_status: Final response - is_tollgate: {}, has_ad: {}",
            response.is_tollgate,
            response.tollgate_advertisement.is_some()
        );

        Ok(response)
    }

    async fn get_current_wifi_info(
        &self,
    ) -> Result<Option<CurrentWifi>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn start_automatic_payment_session(
        &self,
        ...
        // Check if we already have an active session for this tollgate
        {
            ...
            if let Some(existing_session) = &*session {
                if existing_session.tollgate_pubkey == advertisement.tollgate_pubkey {
                    println!(
                        "[Tollgate Payment] Session already exists for tollgate: {}",
                        existing_session.tollgate_pubkey
                    );
                    return Ok(());
                }
            }
        ...
        println!(
            "[Tollgate Payment] Detected new tollgate: {}",
        ...
        let step_size = advertisement
            .step_size
            .as_ref()
            .and_then(|s| s.parse::<u64>().ok())
            .unwrap_or(60000); // Default to 1 minute

        let metric = advertisement
            .metric
            .as_ref()
            .map(|s| s.clone())
        ...
        println!(
            "[Tollgate Payment] Step size: {} {}, Price: {} {} per step",
            step_size, metric, pricing_option.price, pricing_option.unit
        );

        // Make initial payment to establish session
        match self
            .make_initial_tollgate_payment(
                gateway_ip,
                &advertisement.tollgate_pubkey,
                mac_address,
                &pricing_option,
            ...
            .await
        {
            Ok(allotment) => {
                println!(
                    "[Tollgate Payment] Initial payment successful! Received allotment: {} {}",
                    allotment, metric
                );
    ...
    async fn check_automatic_payment(
        &self,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn make_tollgate_payment(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        println!(
            "[Tollgate Payment] Making payment for {} {} to tollgate {}",
            step_size, "milliseconds", tollgate_pubkey
        );
        ...
        println!(
            "[Tollgate Payment] Required amount: {} {} for {} steps",
            required_amount, pricing_option.unit, steps
        );

        // Request payment from wallet service
        match self
            .request_payment_from_wallet(&pricing_option.mint_url, required_amount)
            .await
        {
            Ok(cashu_token) => {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        eprintln!(
                            "[Tollgate Payment] Failed to send payment to tollgate: {}",
                            e
                        );
                    }
                }
            ...
            Err(e) => {
                eprintln!(
                    "[Tollgate Payment] Failed to get payment from wallet: {}",
                    e
                );
            }
        }
    ...
    async fn make_initial_tollgate_payment(
        &self,
        gateway_ip: &str,
        tollgate_pubkey: &str,
        mac_address: &str,
        ...
        println!(
            "[Tollgate Payment] Making initial payment for {} {} to tollgate {}",
            step_size, metric, tollgate_pubkey
        );

        // Trigger captive portal before making payment
        if let Err(e) = self.trigger_captive_portal(gateway_ip).await {
            println!(
                "[Tollgate Payment] Warning: Failed to trigger captive portal: {}",
                e
            );
        }
        ...
        // Send payment to tollgate using TIP-03 HTTP endpoint
        let allotment = self
            .send_payment_to_tollgate(gateway_ip, tollgate_pubkey, mac_address, &cashu_token)
            .await?;
        println!(
            "[Tollgate Payment] Initial payment successful! Received allotment: {} {}",
            allotment, metric
    ...
    async fn make_renewal_payment(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        println!(
            "[Tollgate Payment] Making renewal payment for {} {} to tollgate {}",
            step_size, metric, tollgate_pubkey
        );
            ...
            .await
        {
            Ok(cashu_token) => {
                    ...
                    .await
                {
                    Ok(allotment) => {
                        ...
                        // Update session with new allotment
                        {
                            ...
                            if let Some(session) = &mut *session_guard {
                                ...
                                session.last_payment = std::time::Instant::now();
                                println!(
                                    "[Tollgate Payment] Updated session - Total allotment: {} {}",
                                    session.total_allotment, metric
                                );
                            }
                        }
                    }
                    Err(e) => {
                        eprintln!(
                            "[Tollgate Payment] Failed to send renewal payment to tollgate: {}",
                            e
                        );
                    }
                }
            ...
            Err(e) => {
                eprintln!(
                    "[Tollgate Payment] Failed to get renewal payment from wallet: {}",
                    e
                );
            }
        }
    ...
    async fn trigger_captive_portal(
        &self,
        gateway_ip: &str,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
            ...
            .await
        {
            Ok(response) => {
                println!(
                    "[Captive Portal] Trigger successful, status: {}",
                    response.status()
                );
                Ok(())
            }
    ...
    async fn request_payment_from_wallet(
        &self,
        mint_url: &str,
        amount: u64,
        ...
        println!(
            "[Tollgate Payment] Requesting {} sats from mint: {}",
            amount, mint_url
        );
    ...
    async fn send_payment_to_tollgate(
        &self,
        gateway_ip: &str,
        tollgate_pubkey: &str,
        mac_address: &str,
        ...
        let payment_event = nostr::EventBuilder::new(nostr::Kind::Custom(21000), "")
            .tags(vec![
        ...
        println!(
            "{}",
            serde_json::to_string_pretty(&payment_json).unwrap_or_default()
        );
...
impl<R: Runtime> Androidwifi<R> {
    ...
    pub fn get_mac_address(
        ...
        Ok(MacAddressResponse { mac_address })
    }
    ...
    pub fn get_gateway_ip(&self, _payload: Empty) -> crate::Result<GatewayIpResponse> {
        ...
        Ok(GatewayIpResponse { gateway_ip })
    }
    ...
    pub fn detect_tollgate(&self, _payload: Empty) -> crate::Result<TollgateDetectionResponse> {
        ...
        let _gateway_ip = match default_net::get_default_gateway() {
            ...
            Err(_) => {
                return Ok(TollgateDetectionResponse {
                    is_tollgate: false,
                    advertisement: None,
                })
            }
