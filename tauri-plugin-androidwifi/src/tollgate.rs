...
impl TollgateDetector {
    ...
    pub async fn check_tollgate_at_gateway(&self, gateway_ip: &str) -> Result<TollgateDetectionResponse, Box<dyn std::error::Error>> {
        ...
        match client.get(&url).send().await {
            Ok(response) => {
                println!("[Tollgate Debug] HTTP response status: {}", response.status());
                if response.status().is_success() {
                    ...
                    if let Ok(advertisement_json) = serde_json::from_str::<serde_json::Value>(&text) {
                        ...
                        let advertisement = self.parse_tollgate_advertisement(&advertisement_json)?;
                        ...
                        println!("[Tollgate Debug] Failed to parse response as JSON: {}", text);
                        Ok(TollgateDetectionResponse {
                            is_tollgate: false,
                            advertisement: None,
                        })
                    ...
                    println!("[Tollgate Debug] HTTP request failed with status: {}", response.status());
                    Ok(TollgateDetectionResponse {
                        is_tollgate: false,
                        advertisement: None,
                    })
    ...
    pub fn parse_tollgate_advertisement(&self, json: &serde_json::Value) -> Result<TollgateAdvertisement, Box<dyn std::error::Error>> {
        ...
        let empty_vec = vec![];
        let tags = json.get("tags").and_then(|t| t.as_array()).unwrap_or(&empty_vec);
        ...
        for (i, tag) in tags.iter().enumerate() {
            if let Some(tag_array) = tag.as_array() {
                if let Some(tag_name) = tag_array.get(0).and_then(|t| t.as_str()) {
                    println!("[Tollgate Debug] Tag {}: {} with {} elements", i, tag_name, tag_array.len());
                    match tag_name {
                        "tips" => {
                            tips = tag_array.iter()
                                .skip(1)
                        ...
                        "metric" => {
                            metric = tag_array.get(1).and_then(|v| v.as_str()).map(|s| s.to_string());
                            println!("[Tollgate Debug] Parsed metric: {:?}", metric);
                        }
                        "step_size" => {
                            step_size = tag_array.get(1).and_then(|v| v.as_str()).map(|s| s.to_string());
                        ...
                        "price_per_step" => {
                            ...
                            if tag_array.len() >= 5 {
                                let mint_url = tag_array.get(4).and_then(|v| v.as_str()).unwrap_or("").to_string();
                                let price = tag_array.get(2).and_then(|v| v.as_str()).unwrap_or("").to_string();
                                let unit = tag_array.get(3).and_then(|v| v.as_str()).unwrap_or("").to_string();
